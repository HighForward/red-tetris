<template>
  <div v-if="this.getLobby && this.getLobby.players" class="bg-gray"
       style="box-shadow: 0px 0px 25px 5px rgba(0, 0, 0, 0.35);">
...
<!--              maybe change color here for better UX et know who is who-->
              <div class="flex flex-row items-center">- <span :class="player.id === getLobby.owner.id ? 'text-purple' : 'text-purple'" class="ml-1">{{ player.username }}</span>
                <img v-if="player.id === $socket.client.id" class="pl-2" style="height: 24px" src="@/assets/images/user.png" alt="">
              </div>
            </div>
        ...
        <div class="lobbyRow flex flex-1 flex-col">
          <div class="m-2 p-2 border rounded" style="border-color: #DDDDDD">
            ...
            <div class="flex flex-col font-normal text-smallgray" style="color: #DDDDDD">
              ...
              <div class="flex flex-row justify-between"><span>board</span><span class="text-purple">20 x 10</span>
              </div>
              <div class="flex flex-row justify-between"><span>players</span><span
                class="text-purple">{{ this.getLobby.players.length }}</span></div>
              <div class="flex flex-row justify-between"><span>gamemode</span><span
                class="text-purple">Battle Royal</span></div>
              <div class="flex flex-row justify-between"><span>state</span><span
                class="text-purple">{{ this.getLobby.state }}</span></div>
              <div class="flex flex-row justify-between"><span>uid</span><span class="text-purple">{{
                  this.getLobby.uid
                }}</span></div>
              <div class="flex flex-row justify-between"><span>owner</span><span
                class="text-purple">{{ this.getLobby.owner.username }}</span></div>
            </div>
          </div>
            ...
            <div class="">
              ...
              <div class="flex flex-row justify-around mt-2">
                ...
                <div class="flex flex-col items-center justify-center">
                  <img @click="leaveLobby" @mouseover="infoEnum = 1" @mouseleave="infoEnum = 0"
                       class="imgTetromino cursor-pointer" src="@/assets/images/Tetromino_S.svg">
